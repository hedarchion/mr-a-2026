...
- Before building any worksheet, resolve its exact assessment format (paper, part, task type) against the matching `paper-<paper>-part-<part>-*.md` guide. If the Source of Truth label names no real format, contradicts the requested construct (e.g. a grammar worksheet labelled P1P3, or a part number that does not exist), or two formats plausibly fit, stop and ask one concise format question instead of inferring. Never invent the format and record it as an assumption.
