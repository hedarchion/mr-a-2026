...
All text is plain text, rendered safely as text nodes. Do not pass HTML, CSS, a component tree or scripts. One inline mark is supported: wrap a word as `==word==` to highlight it as a green bold keyword (`.kw`); announcements and exports strip the marks automatically. Keep highlights to target words only. Add `"kwOnReveal": true` to keep highlights hidden as plain text until Reveal, then show them green in place — use for find-it model slides where the highlight is the answer. Every slide needs a unique `id`, supported `type`, and `prompt`. `hint` and `notes` are optional. Public answers are present in the HTML data even before Reveal; this is a teacher-led teaching surface, not a secure test. Notes are excluded from HTML but remain in the JSON/Markdown: never put private pupil data in any publishable source.

For diagnostic and tutoring decks, use one question per slide with stimulus-first ordering: the `stimulus` renders first and dominant, the `prompt` (question stem) follows as subordinate smaller text, and same-slide evidence stays with its question. Label selectable choices `A` / `B` / `C` (`D` for a fourth option). Optional semantic fields `label` and `instruction` render respectively as a subordinate task label and a subordinate administration instruction; state response administration once per deck (opener and/or teacher notes) rather than repeating a per-slide instruction such as `Please write down your answer on paper`. Use root `"mode": "answering"` for neutral teacher-operated response capture; place a `review` slide with `reviewIds` after the full question set so response review and JSON export happen once at the end.
...
| `matrix` | 2–4 `rows`, 2–4 `cols`, one `pairs` entry per row with canonical `col` and optional short `clue`, exact `answer`, `feedback` | Pair each row with one column, then Reveal. Split a 4-gap game across two slides (2 rows each, all options shown) rather than shrinking type |
